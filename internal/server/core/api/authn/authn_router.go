...
func RegisterRoutes(mux *http.ServeMux, system systemHandler, authn *Handler, oidc *authhandler.OIDCHandler, externalIDP *authhandler.ExternalIDPHandler, webAuthn *authhandler.WebAuthnHandler, mfaU2F *authhandler.MFAU2FHandler) {
	...
	mux.HandleFunc("POST /api/authn/v1/password/reset/options", authn.QueryPasswordResetOptions)
	mux.HandleFunc("POST /api/authn/v1/password/reset/start", authn.StartPasswordReset)
	mux.HandleFunc("POST /api/authn/v1/password/reset/finish", authn.FinishPasswordReset)

...
func RegisterRoutes(mux *http.ServeMux, user *Handler, authn authnHandler) {
	...
	mux.HandleFunc("POST /api/user/v1/securekey/update", user.UpdateCurrentUserSecureKey)
	...
	mux.HandleFunc("POST /api/user/v1/recovery_code/query", authn.QueryPortalRecoveryCodes)
...
type authnHandler interface {
	...
	QueryPortalRecoveryCodes(http.ResponseWriter, *http.Request)
